...
public class SecondExercise extends BaseClass {
    ...
    public void mainSecondTest() {
        ...
        // 9. Assert that
        Boolean waterLog;
        waterLog = driver.findElement(By.xpath("//li[contains(.,'Water') and contains(.,'true')]")).isDisplayed();
        assertThat(waterLog).isTrue();
        Boolean windLog = driver.findElement(By.xpath("//li[contains(.,'Wind') and contains(.,'true')]")).isDisplayed();
        assertThat(windLog).isTrue();
        Boolean metalLog;
        metalLog = driver.findElement(By.xpath("//li[contains(.,'metal') and contains(.,'Selen')]")).isDisplayed();
        assertThat(metalLog).isTrue();
        Boolean colorLog;
        colorLog = driver.findElement(By.xpath("//li[contains(.,'Colors') and contains(.,'Yellow')]")).isDisplayed();
        assertThat(colorLog).isTrue();
    }
}
